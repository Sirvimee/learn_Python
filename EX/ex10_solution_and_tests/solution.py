"""Solution to be tested."""


def students_study(time: int, coffee_needed: bool) -> bool:
    """
    Return True if students study in given circumstances.

    (19, False) -> True
    (1, True) -> False.
    """
    if 5 <= time <= 24:
        if 5 <= time <= 17 and coffee_needed is True:
            return True
        if 18 <= time <= 24:
            return True
    return False


def lottery(a: int, b: int, c: int) -> int:
    """
    Return Lottery victory result 10, 5, 1, or 0 according to input values.

    (5, 5, 5) -> 10
    (2, 2, 1) -> 0
    (2, 3, 1) -> 1
    """
    if a == b == c == 5:
        return 10

    if a == b and b == c:
        return 5

    if a != b and a != c:
        return 1

    return 0


def fruit_order(small_baskets: int, big_baskets: int, ordered_amount: int) -> int:
    """
    Return number of small fruit baskets if it's possible to finish the order, otherwise return -1.

    (4, 1, 9) -> 4
    (3, 1, 10) -> -1
    """
    if ordered_amount == 0:
        return 0

    if big_baskets == 0:
        if ordered_amount <= small_baskets:
            return ordered_amount
        return -1

    if ordered_amount < 5:
        if ordered_amount <= small_baskets:
            return ordered_amount
        return -1

    big_basket_needed = ordered_amount // 5

    if big_basket_needed <= big_baskets:
        if big_basket_needed % 5 == 0:
            return 0
        elif big_basket_needed % 5 <= small_baskets:
            return big_basket_needed % 5
        return -1
    elif (big_basket_needed - big_baskets) * 5 <= small_baskets:
        return (big_basket_needed - big_baskets) * 5
    return -1
