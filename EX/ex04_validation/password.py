...
def is_different_from_old_password(old_pass: str, new_pass: str) -> bool:
    ...
    """
    half_length = math.ceil(len(new_pass) / 2)  # Find the number of letters in half a word

    for i in range(len(old_pass) - half_length + 1):
        if old_pass[i:i + half_length].lower() in new_pass.lower() or old_pass[i:i + half_length][::-1] in new_pass:
            return False
...
def is_birthday_in_password(password: str, birthdate: str) -> bool:
    ...
    """
    birthdate_digits = birthdate.split(".")

    # check if the 4-digit full year is in reverse format
    if birthdate_digits[2][::-1] in password:
        return True

    for digits in birthdate_digits:
        if digits[-2:] in password:
            return True
    return False
...
def is_password_valid(new_password: str, old_password: str, name: str, birthdate: str) -> bool:
    ...
    """
    return (
        is_correct_length(new_password) and includes_uppercase(new_password) and
        includes_lowercase(new_password) and includes_special(new_password) and
        includes_number(new_password) and is_different_from_old_password(old_password, new_password) and
        not is_name_in_password(new_password, name) and not is_birthday_in_password(new_password, birthdate)
    )
