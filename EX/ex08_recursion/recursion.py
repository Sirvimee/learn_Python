...
def replace(input_string: str, char_to_replace: str, new_string: str) -> str:
    ...
    """
    if len(char_to_replace) > 1 or len(char_to_replace) == 0:
        return "Length of char_to_replace must be one character!"

    if input_string == "":
        return ""

    return input_string[0].replace(char_to_replace, new_string) + replace(input_string[1:], char_to_replace, new_string)
...
def x_sum_recursion(nums: list, x: int) -> int:
    ...
    :return: sum of every x'th number in the list
    """
    if x == 0 or not nums or x - 1 > len(nums):
        return 0
