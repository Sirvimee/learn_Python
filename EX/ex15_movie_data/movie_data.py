...
class MovieData:
    ...
    def load_data(self, movies_filename: str, ratings_filename: str, tags_filename: str) -> None:
        ...
        tags = pd.read_csv(tags_filename)
        self.movies = movies
    ...
    def create_aggregate_movie_dataframe(self, nan_placeholder: str = '') -> None:
        ...
        ratings = ratings.drop(labels=['userId', 'timestamp'], axis=1)
        ...
        self.aggregate_movie_dataframe = movies_ratings_tags
...
class MovieFilter:
    ...
    def filter_movies_by_rating_value(self, rating: float, comp: str) -> pd.DataFrame | None:
        ...
        if rating is None or rating < 0:
            raise ValueError('None value given as rating')
        if comp != 'greater_than' and comp != 'equals' and comp != 'less_than':
            raise ValueError('comp value not valid')
    ...
    def filter_movies_by_genre(self, genre: str) -> pd.DataFrame:
        ...
        if genre is None or genre == '':
            raise ValueError('None value given as genre')
    ...
    def filter_movies_by_tag(self, tag: str) -> pd.DataFrame:
        ...
        if tag is None or tag == '':
            raise ValueError('None value given as tag')
